...
import subprocess
import conll09_utils

_SPAN_SRL_CONLL_EVAL_SCRIPT  = "scripts/run_conll_eval.sh"
_DEPENDENCY_SRL_CONLL_EVAL_SCRIPT = "scripts/eval_09.pl"
...
def compute_dependency_f1(sentences, gold_srl, predictions, srl_conll_eval_path, disamb_rate):
  ...
  for gold, prediction in zip(gold_srl, predictions):
    ...
    for pred_id, gold_args in gold.iteritems():
      filtered_gold_args = [a for a in gold_args if a[2] not in ["_"]]
      ga += len(filtered_gold_args)
    for pred_id, pred_args in prediction.iteritems():
      filtered_args = [a for a in pred_args if a[2] not in ["_"]]
    ...
    for pred_id, gold_args in gold.iteritems():
      ...
      for a0 in gold_args:
        for a1 in pred_args:
          if a0[0] == a1[0] and a0[2] == a1[2] and a1[2] not in ["_"]:
            ca += 1
  
  precision = (ca + cp * disamb_rate) / (pa + pp)
  recall = (ca + cp * disamb_rate) / (ga + gp)
  f1  = 2 * precision * recall / (precision + recall)
  print('SRL(unofficial) GA:',ga,'GP:',gp,'PA:',pa,'PP:',pp,'CA:',ca,'CP:',cp)
  print ("SRL(unofficial) Precision: {}, Recall: {}, F1: {}".format(precision*100, recall*100, f1*100))
  ...
  #   print_dependency_conll(sentences, gold_srl, gold_path, None)
  ...
  try:
    ...
    print('SRL(official) GA:',conll_ga,'GP:',conll_gp,'PA:',conll_pa,'PP:',conll_pp,'CA:',conll_ca,'CP:',conll_cp)
    print ("SRL(official) Precision: {}, Recall: {}, F1: {}".format(conll_precision*100, conll_recall*100, conll_f1*100))
  except:
    conll_recall = 0
    conll_precision = 0
    conll_f1 = 0
