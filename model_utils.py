...
def bilinear(head, dep, bias_head, bias_dep, out_channels):
  if bias_head:
  ...
  RU = tf.stack(tf.split(slim.fully_connected(head, dep_size * out_channels, None, weights_initializer=tf.zeros_initializer, biases_initializer=tf.zeros_initializer), out_channels, -1), 1)
...
# def biaffine(arg_emb, pred_emb, num_labels, config, dropout):

#   num_sentences = util.shape(arg_emb, 0)
#   num_args = util.shape(arg_emb, 1)
#   num_preds = util.shape(pred_emb, 1)

#   arg_emb_size = util.shape(arg_emb, 2)
#   pred_emb_size = util.shape(pred_emb, 2)

#   with tf.variable_scope("bilinear_predicate_argument_scores"):

#     RU = tf.stack(tf.split(slim.fully_connected(arg_emb, pred_emb_size * num_labels, activation_fn=None, biases_initializer=None, weights_initializer=tf.zeros_initializer()), num_labels, -1), 1)

#     bilinear_predicate_argument_scores = tf.matmul(RU, tf.stack([tf.transpose(pred_emb, [0, 2, 1])] * num_labels, 1))

#     bilinear_predicate_argument_scores = tf.transpose(bilinear_predicate_argument_scores, [0, 2, 3, 1])
...
#   linear_predicate_argument_scores = get_unary_scores(flat_pair_emb, config, dropout, num_labels,
#       "linear_predicate_argument_scores")  # [num_sentences * num_args * num_predicates, num_labels]

#   linear_predicate_argument_scores = tf.reshape(linear_predicate_argument_scores, [num_sentences, num_args, num_preds, num_labels])



#   predicate_argument_scores = bilinear_predicate_argument_scores + linear_predicate_argument_scores

#   return predicate_argument_scores
...
def get_softmax_loss(scores, labels, candidate_mask=None):
