...
class LSGNEvaluator(object):
  ...
  def evaluate(self, session, data, predictions, loss, official_stdout=False):
    ...
    for i, doc_tensors in enumerate(self.eval_tensors):
      ...
      if "srl" in decoded_predictions:
        ...
        for j in range(len(sentences)):
          ...
          # just for predicate F1, not harm to CoNLL09
      ...
      if (i + 1) % 50 == 0 and not self.quiet:
    ...
    if self.config["srl_mode"] == 'span':
      ...
      precision, recall, f1, conll_precision, conll_recall, conll_f1 = (srl_eval_utils.compute_dependency_f1(sentences, gold_srl, srl_predictions, self.config["srl_conll_eval_path"], self.config["pred_disamb_rate"]))
      task_to_f1["srl"] = conll_f1
      summary_dict["PAS F1"] = f1
      summary_dict["PAS precision"] = precision
      summary_dict["PAS recall"] = recall
    ...
    if total_num_predicates > 0 and not self.quiet:
    ...
    if total_gold_predicates > 0 and not self.quiet:
